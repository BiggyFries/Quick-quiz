...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: profile }, { data: settings }, { data: attempts }, { data: achievements }] = await Promise.all([
      admin.from('profiles').select('id,role,character').eq('id', user.id).single(), admin.from('user_settings').select('*').eq('user_id', user.id).single(), admin.from('attempts').select('*').eq('user_id', user.id).order('started_at'), admin.from('user_achievements').select('code,unlocked_at,attempt_id').eq('user_id', user.id).order('unlocked_at'),
    ]);
    ...
    return json({ profile: { id: profile.id, role: profile.role, character: profile.character, settings: { sound: settings.sound, music: settings.music, reducedMotion: settings.reduced_motion, highContrast: settings.high_contrast, vibration: settings.vibration }, attempts: history }, stats, achievements: achievements ?? [], history });
